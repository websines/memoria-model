...
class TestLightningIndexer:
    ...
    def test_compute_scores_shape(self):
        ...
        scores = indexer.compute_scores(x)
    ...
    def test_select_topk(self):
        ...
        x = torch.randn(2, 16, 128)
        scores = indexer.compute_scores(x)
        indices = indexer.select_topk(scores, k=4)
    ...
    def test_topk_clamps_to_seq_len(self):
        ...
        scores = indexer.compute_scores(x)
        indices = indexer.select_topk(scores, k=100)
    ...
    def test_ste_quantization_during_training(self):
        ...
        indexer.train()
        x = torch.randn(1, 8, 128)
        scores_train = indexer.compute_scores(x)

        indexer.eval()
        scores_eval = indexer.compute_scores(x)
...
class TestBeliefConditioning:
    def test_no_beliefs(self):
        ...
        x = torch.randn(1, 8, 128)
        scores = indexer.compute_scores(x, beliefs=None)
    ...
    def test_with_beliefs(self):
        ...
        scores_no_belief = indexer.compute_scores(x, beliefs=None)
        scores_with_belief = indexer.compute_scores(x, beliefs=beliefs)
    ...
    def test_empty_beliefs(self):
        ...
        scores = indexer.compute_scores(x, beliefs=empty_beliefs)
        assert scores.shape == (1, 8, 8)
...
class TestMLADSAIntegration:
    ...
    def dsa_config(self):
        ...
        config.transformer.dsa_top_k_ratio = 0.5
