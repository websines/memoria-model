...
def test_read_empty_state(interface, state):
    ...
    output, candidates, _ = interface(hidden, state)
...
def test_read_retrieves_relevant_beliefs(config, state):
    ...
    output, utility = read_path(hidden, state)
...
def test_read_with_goal_modulation(config, state):
    ...
    out_no_goal, _ = read_path(hidden, state)
    out_with_goal, _ = read_path(hidden, state, goal_embeddings=goal_embed, goal_priorities=goal_priorities)
...
def test_interface_gradients_flow(interface, state):
    ...
    output, _, _ = interface(hidden, state)
    loss = output.sum()
    loss.backward()
