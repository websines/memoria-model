...
CURATED_SOURCES: list[DataSource] = [
    ...
    # Nemotron Terminal Corpus (cleaned): terminal coding agent SFT
    DataSource(
        name="nemotron_terminal",
        hf_id="AmanPriyanshu/tool-reasoning-sft-CODING-Nemotron-Terminal-Corpus-data-cleaned-rectified",
        weight=0.04,
...
def curated_stream(
    ...
    """Stream tokenized sequences from the curated multi-tier dataset mix.
    ...
    fineweb_iter = stream_fineweb_edu(tokenizer, seq_len, skip_documents=skip_documents)
    code_iter = _try_code_stream(tokenizer, seq_len, code_languages)

    # Synthetic stream
    synthetic_iter = _synthetic_text_stream(synthetic_data)

    # Tokenization buffer (shared across all text sources)
    eos_id = tokenizer.eos_token_id
    if eos_id is None:
        eos_id = tokenizer.vocab_size - 1
    buffer: list[int] = []

    print(f"  Curated stream: {len(active_curated)} datasets + FineWeb + code + synthetic")
    ...
    while True:
        ...
        # Tokenize and pack into sequences
...
def _try_code_stream(tokenizer, seq_len, languages):
    """Try to start a code stream, return None on failure."""
    try:
        return stream_code(tokenizer, seq_len, languages=languages)
    except Exception as e:
