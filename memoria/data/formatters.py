...
def format_openr1_math(sample: dict) -> str:
    """OpenR1-Math-220k: math problems with multiple DeepSeek R1 reasoning traces.

    Each problem has 2-4 verified solution attempts. We pick the first correct one
    for NTP. The multiple traces per problem provide natural revision signal —
    the model sees that different reasoning paths reach the same answer.
    """
    problem = sample.get('problem', '')
    if not problem:
        return ''
    ...
    for i, gen in enumerate(generations):
        if i < len(correctness) and correctness[i]:
            solution = gen
            break

    # Fallback: use the first generation or the 'solution' field
    if not solution:
        if generations:
            solution = generations[0]
        else:
...
def format_nemotron_pretraining(sample: dict) -> str:
    """Nemotron-Pretraining-Specialized-v1.1: 9.3B tokens of synthetic pretraining.

    Code concepts (15M samples with concept taxonomy), formal logic (491K),
    economics (345K), MMLU-style MCQ (3.5M). Generated by gpt-oss/Qwen3/DeepSeek-v3.
    """
    return sample.get('text', '')


def format_xhigh_reasoning(sample: dict) -> str:
    """GPT-5.4 XHIGH Reasoning: 2.7K elite samples with deep CoT traces.

    ~12,600 char thinking per sample across 60+ expert domains (math, code,
    science, algebraic topology, CRISPR, game theory, ZK-proofs...).
    Explicit <thinking> blocks ideal for refinement loop internalization.
    """
...
def format_nemotron_science(sample: dict) -> str:
    """Nemotron-Science-v1: 226K scientific reasoning QA with reasoning traces.

    MCQA (GPQA-style) + chemistry reasoning covering genetics, pharmacogenomics,
    CRISPR, RNA-seq, synthetic biology. Reasoning traces teach structured
    scientific thinking — good for refinement loop internalization.
    """
    messages = sample.get('messages', [])
    if not messages:
        return ''
    ...
    for msg in messages:
        ...
        if reasoning:
            parts.append(f"{role}: <think>{reasoning}</think>\n{content}")
        elif content:
            parts.append(f"{role}: {content}")
    return "\n".join(parts)


def format_hermes_agent_traces(sample: dict) -> str:
    """Hermes Agent Reasoning Traces: multi-turn tool-calling trajectories.

    14.7K real agent runs with <think> reasoning blocks, <tool_call> invocations,
    and <tool_response> results. Avg 24 turns, 14 tool calls per sample.
    Categories: terminal, browser, multi-tool, planning, file ops.

    Preserves <think>/<tool_call>/<tool_response> tags — refinement loops should
    internalize the reasoning patterns, beliefs should track tool results,
    and the goal system should learn multi-step task decomposition.
    """
    return _format_with_tools(sample, turns_key='conversations', tools_key='tools')
